...
const StackModal = () => {
  ...
  return createPortal(
    ...
    >
      ...
      <section
        ref={contentRef}
        className="relative w-full h-fit overflow-hidden [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden bg-white" // Added bg-white
      >
        {stackModalContent && (
          <div className="w-full h-fit transition-opacity duration-700 ease-in-out">{stackModalContent}</div>
        )}
      </section>
    </dialog>,
